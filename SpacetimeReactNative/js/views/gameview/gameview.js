...
var Player = require('../playerview/playerview.js');
...
let GameView = React.createClass({
    ...
    let opponentBoardCards, homePlayerBoardCards;
    if (opponent.board) {
      opponentBoardCards = opponent.board.map((cardInfo, i) =>
          <Card cardInfo={cardInfo} player={opponent} key={i} />);

    }
    if (homePlayer.board) {
       homePlayerBoardCards = homePlayer.board.map((cardInfo, i) =>
    ...
    return (
        <View className="game-container">
          ...
          <View onClick={this.selectOpponent} className="player-avatar" style={this.avatarStyle(1).avatar}>
